...
import ModelY from "../../assets/images/shop/tesla-shop-marketing/ModelY.avif";

export function ShopHome({ desktopImage, alt, name, textColor,mobileImage }) {
  return (
    <div className="relative h-screen w-full">
      ...
      <div className="absolute bottom-19 left-0 right-0 z-10 flex flex-col items-center gap-5 px-4 ">
        <h2 className= {`text-[24px] md:text-[40px] font-normal   text-center ${textColor}   font-medium md:font-medium md:mb-3 mb-3`}  >
          {name}
        </h2>

        <button className=" bg-white rounded-sm text-[14px] font-normal px-10 py-2 w-full max-w-md md:w-[283px]  text-black font-bold">
          Shop now
        </button>
      </div>
    </div>
...
// this is for rendering all the images
...
export function Rendering() {
  ...
  return (
    <>
      <ul>
        {forRendering.map((render) => {
          return (
            <ShopHome 
            desktopImage={render.desktopImage}
            mobileImage={render.mobileImage} 
            alt={render.er} 
            name={render.title} 
